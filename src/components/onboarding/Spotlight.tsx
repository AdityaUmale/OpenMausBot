...
//
// Motion: the cutout eases from the full window down to the anchor, and the
// card scales in from the anchor's side. Under reduced motion both appear.
// With no anchor the card sits centred over the dimmed window.
...
function measure(anchor: string): Rect | null {
  const all = Array.from(document.querySelectorAll<HTMLElement>(`[data-tour="${anchor}"]`)).filter(
...
export function Spotlight({
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      frame = requestAnimationFrame(() => setRect(measure(anchor)));
    };
    update();
  ...
  let below = true;
  let beside = false;
  if (rect && placement === "right" && rect.x + rect.w + GAP + CARD_W <= viewportW - 12) {
    // beside a sidebar control, centred on it, kept clear of the window edges
    beside = true;
    style = { left: rect.x + rect.w + GAP, top: Math.max(12, Math.min(rect.y + rect.h / 2 - 70, viewportH - 180)) };
  } else if (rect) {
    ...
    below = placement === "below" ? roomBelow >= 140 || roomAbove < roomBelow : roomAbove < 140 && roomBelow > roomAbove;
    if (roomBelow < 140 && roomAbove < 140) {
      // the anchor fills the window (a panel, a page): sit inside it, top right
      below = true;
      style = { left: Math.max(12, Math.min(rect.x + rect.w - CARD_W - GAP * 2, viewportW - CARD_W - 12)), top: rect.y + GAP * 2 };
    } else {
      style = below ? { left, top: rect.y + rect.h + GAP } : { left, bottom: viewportH - rect.y + GAP };
    }
  } else {
  ...
  return createPortal(
    <div className="pointer-events-none fixed inset-0 z-[60]" aria-live="polite">
      ...
      <div
        ...
        data-tour-card
        className={cn(
          "pointer-events-auto absolute flex w-[320px] items-start gap-3 rounded-2xl border border-hairline/50 bg-panel p-3.5 shadow-[0_24px_60px_-20px_rgba(0,0,0,0.6)]",
          rect ? (beside ? "origin-left" : below ? "origin-top-left" : "origin-bottom-left") : "origin-center",
          settled ? "animate-spot-in" : "opacity-0",
        )}
        style={style}
      >
        <div className="shrink-0 drop-shadow-[0_6px_14px_rgba(0,0,0,0.35)]">
          <MausAvatar color="green" state={mascot} size={38} trackPointer={false} />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2.5 flex items-center gap-3">
            ...
            {progress && <span className="text-[11.5px] tabular-nums text-ink-secondary">{progress}</span>}
            {secondary && (
              <button type="button" onClick={secondary.onClick} className="ml-auto text-[12px] text-ink-secondary transition-colors hover:text-ink">
                {secondary.label}
              </button>
            )}
          </div>
