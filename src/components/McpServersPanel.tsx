...
export function parseMcpEnvironment(
  ...
  for (const original of value.split(/\r?\n/)) {
    ...
    if (equals <= 0) return { ok: false, error: t("mcp.env.useKeyValue", { line }) };
    const key = line.slice(0, equals).trim();
    const secret = line.slice(equals + 1);
    if (!ENV_NAME.test(key)) return { ok: false, error: t("mcp.env.invalidName", { key }) };
    if (Object.hasOwn(env, key)) return { ok: false, error: t("mcp.env.duplicate", { key }) };
...
export function McpServersPanel() {
  ...
  const save = async () => {
    ...
    if (!name || !command) {
      setError(t("mcp.err.nameAndCommand"));
      return;
    }
    ...
    try {
      ...
      setNotice(editing === "new" ? t("mcp.saved", { name }) : t("mcp.updated", { name }));
      closeEditor();
    } catch (cause) {
