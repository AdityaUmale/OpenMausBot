...
function nextRunLabel(at: number | null) {
  ...
  return `${sameDay ? t("chat.day.today") : date.toLocaleDateString([], { month: "short", day: "numeric" })}, ${date.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`;
}
...
export function ComputerPanel({
  ...
  useEffect(() => {
    ...
    if (bot.computer === "local") {
      if (!providerSupportsLocal) {
        setError(t("computer.err.localEngine"));
      }
    ...
    if (bot.computer === "vm") {
      if (!vmSupported) {
        setError(t("computer.err.vmEngine"));
      ...
      api(`/api/bots/${bot.id}/local-computer`)
        .then((rawStatus) => {
          ...
          if (status.ready) {
            ...
            setError(canCreateHere ? null : t("computer.err.vmOpenSettings", {
              problem: status.problem ?? t("computer.err.vmNotReady"),
            }));
            setPhase("vm-unavailable");
          }
        })
    ...
    if (bot.computer === "cloud" && !cloudSupported) {
      setError(t("computer.err.cloudEngine"));
      setPhase("error");
      return;
    }
    ...
    if (cloudBackend === "vps") {
      ...
      if (!vpsSupported) {
        if (autoLocal) setPhase("local");
          ...
          setError(t("computer.err.vpsEngine"));
          setPhase("error");
        }
        return;
      }
        ...
        .then((rawStatus) => {
          ...
          if (!status.configured) {
            if (autoLocal) setPhase("local");
              ...
              setError(t("computer.err.vpsAlias"));
          ...
          if (bot.computer === "cloud") {
            ...
            return api(`/api/bots/${bot.id}/computer/provision`, { method: "POST" }).then((result) => {
              ...
              if (result.ready) {
                ...
                setError(result.problem ?? t("computer.err.vpsNotReady"));
                setPhase("error");
              }
            });
          }
          ...
          setError(
            bot.autoStartVps
              ? t("computer.err.vpsAuto", { problem: status.problem ?? t("computer.err.vpsNoContainer") })
              : t("computer.err.vpsManual", { problem: status.problem ?? t("computer.err.vpsNoContainer") }),
          );
    ...
    const shoot = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted && latestLive.current.at <= startedAt) {
          if (typeof png !== "string" || !png.trim()) throw new Error(t("computer.err.emptyFrame"));
        ...
        if (!controller.signal.aborted && latestLive.current.at <= startedAt) {
          setPreviewError(e instanceof Error && e.name === "TimeoutError"
            ? t("computer.err.frameTimeout")
            : e instanceof Error ? e.message : t("computer.err.screenUnavailable"));
        }
      } finally {
        inFlight = false;
      }
  ...
  const openDesktop = async () => {
    ...
    try {
      ...
      if (!viewerUrl) throw new Error(t("computer.err.noDesktopLink"));
      ...
      if (window.ogb?.desktopViewer) {
        ...
        if (!opened) throw new Error(t("computer.err.openDesktopLink"));
      } else if (!window.open(viewerUrl, "_blank", "noopener")) {
        throw new Error(t("computer.err.popupBlocked"));
      }
    } catch (e) {
      fallbackTab?.close();
  ...
  const replaceVpsComputer = async () => {
    ...
    try {
      ...
      if (!result.ready) setError(result.problem ?? t("computer.err.vpsReplaceNotReady"));
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setPhase("error");
    } finally {
